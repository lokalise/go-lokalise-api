...
func TestClient_Projects_Delete(t *testing.T) {
	...
	type output struct {
		calledPath  string
		requestbody string
		response    model.ProjectDeleteResponse
		err         error
	}
	type serverResponse struct {
		body       string
	...
	tt := []struct {
	...
	}{
		{
			...
			output: output{
				calledPath:  "/projects/3002780358964f9bab5a92.87762498",
				requestbody: ``,
				response: model.ProjectDeleteResponse{
					ProjectID: "3002780358964f9bab5a92.87762498",
					Deleted:   true,
	...
	for _, tc := range tt {
		t.Run(tc.name, func(t *testing.T) {
			...
			server := httptest.NewServer(http.HandlerFunc(func(rw http.ResponseWriter, req *http.Request) {
				...
				requestbody, err = ioutil.ReadAll(req.Body)
				assert.NoError(err, "read request body failed")
				assert.Equal(req.Method, "DELETE", "wrong HTTP request verb")
				rw.Header().Set("Content-Type", "application/json")
				rw.WriteHeader(tc.serverResponse.statusCode)
				fmt.Fprintf(rw, tc.serverResponse.body)
			}))
			...
			resp, err := client.Projects.Delete(context.Background(), tc.input.projectID)
			...
			assert.Equal(tc.output.requestbody, string(requestbody), "call body no as expected")
			assert.Equal(tc.output.response.ProjectID, resp.ProjectID, "response project id not as expected")
			assert.Equal(tc.output.response.Deleted, resp.Deleted, "response project deleted not as expected")
		})
	}
}
